...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	domain := r.FormValue("team_domain")
	...
	if command == "/webex" {
		...
		if text == "help" {
			...
			jsonStr = []byte(`{"response_type":"in_channel", "text": "` + user + ` has set up a WebEx room to join", "attachments":[{"text":"https://` + domain + `.webex.com/join/` + user + `","color":"#3AA3E3"}]}`)
		}
